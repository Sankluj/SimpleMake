...
int linesBin_parseLine(char type, char *line, struct linesBin *lb) {
	...
	switch(type) {
		...
		case 'B' :	linesBin_addLine(&(lb->lib_ctr), line, lb->libLine);
					break;
		case '#' :	prints("Commment : ", line);
					break;
		default  :	print("Non reconnu \n");
					break;
	} 
	errCode = 0;
	return errCode;
...
int linesBin_check(struct linesBin *lb) {
	int errCode;
	if(lb->exec_ctr == 2){
		errCode = ERROR_NO_EXEC;
		return errCode;
	}
	if (lb->src_ctr == 3){
		errCode = ERROR_NO_SRC;
		return errCode;
	}
	errCode = 0;
	return errCode;
...
void linesBin_printer(struct linesBin *lb) {
	...
	while (cc < lb->src_ctr) {
		prints("Cline: ", lb->srcLine[cc++]);
	}
	while (hc < lb->head_ctr) {
		prints("Hline: ", lb->headLine[hc++]);
	}
	while (fc < lb->flags_ctr) {
		prints("Fline: ", lb->flagsLine[fc++]);
	}
	while (bc < lb->lib_ctr) {
		prints("Bline: ", lb->libLine[bc++]);
	}
}
